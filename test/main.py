...
def get_args():
    ...
    parser.add_argument('--fe_ckpt', type=str, default='path/to/feature_extractor.pth', help='Path to feature extractor checkpoint (.pth)')
    parser.add_argument('--classifier1_ckpt', type=str, default='path/to/classifier1.pth', help='Path to classifier 1 checkpoint (.pth)')
    parser.add_argument('--classifier2_ckpt', type=str, default='path/to/classifier2.pth', help='Path to classifier 2 checkpoint (.pth)')
    parser.add_argument('--classifier3_ckpt', type=str, default='path/to/classifier3.pth', help='Path to classifier 3 checkpoint (.pth)')
...
def main():
    ...
    for p in [args.fe_ckpt, args.classifier1_ckpt, args.classifier2_ckpt, args.classifier3_ckpt]:
        if not Path(p).exists():
            raise FileNotFoundError(
                f"Checkpoint not found: {p}. Please pass the correct path (e.g., --fe_ckpt path/to/...)."
            )

    fe = torch.load(args.fe_ckpt, map_location='cpu')
    vgg19_1 = torch.load(args.classifier1_ckpt, map_location='cpu')
    vgg19_2 = torch.load(args.classifier2_ckpt, map_location='cpu')
    vgg19_3 = torch.load(args.classifier3_ckpt, map_location='cpu')

    task_model = TaskModel(fe, vgg19_1, vgg19_2, vgg19_3, False)
