...
def main():
    ...
    fe = torch.load('/home/rrgaire/projects/iccv/paper/cifar100/densenet121/indv/checkpoints/CIFAR100_DenseNet121_indv_fe.pth')
    densenet121 = torch.load('/home/rrgaire/projects/iccv/paper/cifar100/densenet121/indv/checkpoints/CIFAR100_DenseNet121_indv_classifier.pth')
    fe = fe.to(args.device)
    densenet121 = densenet121.to(args.device)
    log_msg = {}
    base_macs, base_nparams = tp.utils.count_ops_and_params(densenet121, example_inputs)
    log_msg['Base FLOPs'] = 2 * base_macs
    log_msg['Base Params'] = base_nparams

    # task_model_o = TaskModel(fe, densenet121, False)
    # eval_loss, eval_accuracy, time_elapsed = evaluate_model(
    #         args=args,
    #         task_model=task_model_o,
    #         loader=test_loader,
    #         criterion=criterion
    #     )
    # print(eval_accuracy)

    pruned_model = prune_model(args, densenet121, args.ratio, example_inputs)

    base_macs, base_nparams = tp.utils.count_ops_and_params(pruned_model, example_inputs)

    log_msg['Pruned FLOPs'] = 2 * base_macs
