...
def evaluate_task(args, sampler, task_model, loader, input):
    ...
    return accuracy, total_inference_time, total_flops / 10000, total_params / 10000, flops_sampler, params_sampler
...
def main():
    ...
    task_model = torch.load('/home/rrgaire/projects/iccv/paper/tinyimagenet/densenet121/sampler/checkpoints/task_model.pth')
    task_model = task_model.to(args.device)

    sampler = Sampler(num_modules=3)
    sampler.load_state_dict(torch.load('/home/rrgaire/projects/iccv/paper/tinyimagenet/densenet121/sampler/checkpoints/TI_DNET_RFC__sampler.pth'))
    ...
    log_msg["Task Module 1, FLOPs"] = ((flops[0] + flops[1])) / 1e8
    log_msg["Task Module 1, Params"] = ((params[0] + params[1])) / 1e6
    log_msg["Task Module 2, Accuracy"] = acc[1] * 100
    log_msg["Task Module 2, FLOPs"] = ((flops[0] + flops[2])) / 1e8
    log_msg["Task Module 2, Params"] = ((params[0] + params[2])) / 1e6
    log_msg["Task Module 3, Accuracy"] = acc[2] * 100
    log_msg["Task Module 3, FLOPs"] = ((flops[0] + flops[3])) / 1e8
    log_msg["Task Module 3, Params"] = ((params[0] + params[3])) / 1e6
    ...
    log_msg["Task Task FLOPs"] = task_flops / 1e8
    log_msg["Task Task Params"] = task_params / 1e6
    log_msg["Sampler FLOPS"] = flops_sampler / 1e8
