...
class TinyImageNetWithSampler(torch.utils.data.Dataset):
    def __init__(self, args, model, transform, train=True):
        if train:
            self.cifar100 = torchvision.datasets.ImageFolder('../../dataset/train', transform=transform)
        else:
            self.cifar100 = torchvision.datasets.ImageFolder('../../dataset/val', transform=transform)
    ...
    def compute_energy_costs(self):
        sample_input = torch.randn(1, 128, 16, 16).to(self.device)
    ...
    def compute_sample_metrics(self):
        ...
        with torch.no_grad():
            for i in tqdm(range(len(self.cifar100))):
