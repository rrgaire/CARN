...
def main():
    ...
    example_inputs = torch.randn(1, 256, 32, 32)
    example_inputs = example_inputs.to(args.device)

    # fe = torch.load('/home/rrgaire/projects/iccv/paper/cifar10/resnet50/indv/checkpoints/CIFAR10_ResNet50_indv_fe.pth', map_location='cpu')
    # classifier = torch.load('/home/rrgaire/projects/iccv/paper/cifar10/resnet50/indv/checkpoints/CIFAR10_ResNet50_indv_classifier.pth', map_location='cpu')
    fe = CommonFE(Bottleneck, 3)
    fe.load_state_dict(torch.load('/home/rrgaire/projects/iccv/paper/cifar10/resnet50/indv/checkpoints/resnet50_feature_extractor.pth'))
    classifier = ResNet50(num_classes=args.num_classes)
    classifier.load_state_dict(torch.load('/home/rrgaire/projects/iccv/paper/cifar10/resnet50/indv/checkpoints/resnet50_classifier.pth'))
    ...
    base_macs, base_nparams = tp.utils.count_ops_and_params(classifier, example_inputs)

    for i in range(iterative_steps):
        pruner.step()

        macs, nparams = tp.utils.count_ops_and_params(classifier, example_inputs)
        logger.info(
        ...
        task_model = TaskModel(fe, classifier, False)
